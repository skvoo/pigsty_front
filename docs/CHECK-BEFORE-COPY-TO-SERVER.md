...
Перед выполнением `scp pigsty.yml st@104.223.25.234:~/pigsty/` или применением плейбуков — **сначала проверить версию на сервере и дать отчёт**. При расхождениях — не копировать до согласования.

---

## 0. Обязательная проверка перед копированием

1. **На сервере:** `wc -c ~/pigsty/pigsty.yml` и `md5sum ~/pigsty/pigsty.yml` (или `sed -n '79,84p' ~/pigsty/pigsty.yml` для секции pgb_hba_rules).
2. **Локально:** размер файла и `(Get-FileHash -Path .\pigsty.yml -Algorithm MD5).Hash` (PowerShell).
3. **Сравнить:** если размер и MD5 совпадают — расхождений нет, копировать не нужно. Если отличаются — составить отчёт (см. ниже) и не копировать без подтверждения.
...
### 3.1 Проверка 25.02.2026

| Проверка | Сервер | Репо (локально) | Расхождение |
|----------|--------|------------------|-------------|
| Размер pigsty.yml | 19 058 байт | 19 058 байт | Нет |
| MD5 | 222a1b72bb4d8851b3a770ecde9479f0 | 222A1B72BB4D8851B3A770ECDE9479F0 | Нет |
| pgb_hba_rules (фрагмент) | dbuser_app app, void, tdadmin, **dbuser_app imperialdb (order 103)** | То же | Нет |

**Вывод:** конфиг на сервере и в репо совпадают. Копирование не требуется.

### 3.2 Отчёт при расхождениях

Если размер или MD5 отличаются — **не копировать** до разбора. Рекомендуется:
- Вывести на сервере: `diff -u <(curl -sL ...) ~/pigsty/pigsty.yml` или скопировать серверный файл локально и сделать `diff pigsty.yml pigsty_server.yml`.
- Зафиксировать в отчёте: какие секции отличаются (pgb_hba_rules, pg_databases, minio_*, пароли и т.д.).
- Либо внести только нужные изменения вручную на сервере, либо после подтверждения перезаписать конфиг (предварительно сделать бэкап на сервере).

### 3.3 Архив: проверка 24.02.2026

| Проверка | Результат |
|----------|------------|
| pigsty.yml на сервере | 319 строк, 18937 байт |
| MD5 сервер vs репо | Совпадали (на тот момент 611ed0d88c75b349303b297ef2166658) |
